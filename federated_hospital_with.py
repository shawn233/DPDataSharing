...
n=10 #num of users
####load
a1=data=np.genfromtxt(fname='../data_from_chong_xiang_mlaas/dp_data/pudf/a1.matrix',delimiter='\t',dtype=float,autostrip=True)[:,:-1]
b1=data=np.genfromtxt(fname='../data_from_chong_xiang_mlaas/dp_data/pudf/b1.matrix',delimiter='\t',dtype=float,autostrip=True)[:,:-1].reshape(400)
def encoded(data):
	data=data.cpu().numpy()
	data=np.reshape(data,(-1,776))
	data=np.dot(data,a1)+b1
	data=np.reshape(data,(-1,400))
